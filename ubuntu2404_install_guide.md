...
5. [初期設定](#5-初期設定)
   ...
   - [5-4. ネットワーク設定の確認（DHCP）](#5-4-ネットワーク設定の確認dhcp)
   - [5-5. ファイアウォールの設定（ufw）](#5-5-ファイアウォールの設定ufw)
   - [5-6. 日時設定（NTP）](#5-6-日時設定ntp)
   - [5-7. 日本語入力の設定](#5-7-日本語入力の設定ibus--mozc)
   - [5-8. NVIDIAドライバの設定](#5-8-nvidiagラフィックドライバの設定)
   - [5-9. SSD向け推奨設定](#5-9-ssd向け推奨設定)
   - [5-10. ソフトウェアのインストール](#5-10-ソフトウェアのインストール)
   - [5-11. OpenSSH サーバの設定](#5-11-openssh-サーバの設定)
...
```bash
# 現在取得しているIPアドレスをコマンドで確認する（hpc で実行）
ip a
```
